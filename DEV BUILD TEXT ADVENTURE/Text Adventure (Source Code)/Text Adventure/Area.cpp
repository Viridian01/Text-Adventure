...
Area::Area(string setName, string setDesc) {
	name = setName;
	description = setDesc;
}

void Area::addAccessibleArea(string areaToAdd) {
	for (unsigned int i = 0; i < area.allAreas.size(); i++) {
		if (area.allAreas[i].name == areaToAdd) {
			accessibleAreas.push_back(area.allAreas[i]);
		}
	}
}

void Area::addArea(Area areaToAdd) {
	area.allAreas.push_back(areaToAdd);
}
...
int Area::checkAreaObjects(string objectToCheck) {
	if (areaObjects.size() == 0) {
		cout << "There are no objects in: " << name << endl;
		return -1;
	}
	else {
		for (unsigned int i = 0; i < areaObjects.size(); i++) {
			if (areaObjects[i].name == objectToCheck) {
				return i;
			}
			else if (i == areaObjects.size() - 1) {
				cout << "There is no: " << objectToCheck << " in " << name << endl;
				return -1;
			}
		}
	}
