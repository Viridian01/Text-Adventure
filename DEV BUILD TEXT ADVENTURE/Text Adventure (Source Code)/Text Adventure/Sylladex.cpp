...
void Sylladex::useItemOnObject(string itemToUse, string objectToUseItemOn) {
	int itemPos = checkItem(itemToUse);
	int objPos = player.currentArea.checkAreaObjects(objectToUseItemOn);
	if (itemPos > -1 && objPos > -1) {
		storage[itemPos]->useItemOnObject(player.currentArea.areaObjects[objPos]);
	}
}